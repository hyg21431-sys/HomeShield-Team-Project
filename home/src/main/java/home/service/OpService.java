...
public class OpService { 
    ...
    // 생성자 주입
    public OpService(OpClient opClient) {
        this.opClient = opClient;
    }

    /**
     * 지역 코드(lawdCd), 기준 계약 연월(dealYmd), 조회 기간(period)을 받아 
     * 해당하는 모든 거래 데이터를 조회하고 합쳐서 반환합니다.
     * lawdCd가 '30000'일 경우 대전 전체 구의 데이터를 합쳐서 처리합니다.
     */
    public List<OpItemDTO> getOpLeaseStats(String lawdCd, String dealYmd, int period) {
        
        // 1. 조회할 법정동 코드 리스트를 결정합니다.
        List<String> targetLawdCds;
        if ("30000".equals(lawdCd)) {
            targetLawdCds = DAEJEON_DISTRICT_CODES;
        } else {
            targetLawdCds = Collections.singletonList(lawdCd);
        }
        ...
        List<OpItemDTO> rawData = new ArrayList<>();
        
        for (String targetCd : targetLawdCds) {
            for (String targetYmd : targetDealYmds) {
                // OpClient의 오피스텔 전용 메서드 호출
                List<OpItemDTO> data = opClient.fetchOpLeaseData(targetCd, targetYmd); 
                rawData.addAll(data); 
            }
        }
