...
import home.client.AptClient;
import home.dto.AptItemDTO;      
...
public class AptService { 
    ...
    private final AptClient homeClient;

    // 생성자 주입
    public AptService(AptClient homeClient) {
     ...
     */
    public List<AptItemDTO> getAptLeaseStats(String lawdCd, String dealYmd, int period) { 
        ...
        List<AptItemDTO> rawData = new ArrayList<>();
        ...
        for (String targetCd : targetLawdCds) { // 외부 루프: 지역 코드 순회
            for (String targetYmd : targetDealYmds) { // 내부 루프: 월별 코드 순회
                ...
                List<AptItemDTO> data = homeClient.fetchAptLeaseData(targetCd, targetYmd);
                rawData.addAll(data); 
            }
        }
