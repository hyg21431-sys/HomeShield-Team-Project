...
public class HomeService { 
    ...
    // 생성자 주입
    public HomeService(HomeClient homeClient) {
     ...
     */
    public List<ItemDTO> getAptLeaseStats(String lawdCd, String dealYmd, int period) { 
        ...
        List<ItemDTO> rawData = new ArrayList<>();
        ...
        for (String targetCd : targetLawdCds) { // 외부 루프: 지역 코드 순회
            for (String targetYmd : targetDealYmds) { // 내부 루프: 월별 코드 순회
                ...
                List<ItemDTO> data = homeClient.fetchAptLeaseData(targetCd, targetYmd);
                rawData.addAll(data); 
            }
        }
