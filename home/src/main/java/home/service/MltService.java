...
public class MltService {
    ...
    private final MltClient homeClient;
    
    // 생성자 주입
    public MltService(MltClient homeClient) {
        this.homeClient = homeClient;
    }
    
public List<MltItemDTO> getMltLeaseStats(String lawdCd, String dealYmd, int period) { 
        ...
        List<MltItemDTO> rawData = new ArrayList<>();
        ...
        for (String targetCd : targetLawdCds) { // 외부 루프: 지역 코드 순회
            for (String targetYmd : targetDealYmds) { // 내부 루프: 월별 코드 순회
                ...
                List<MltItemDTO> data = homeClient.fetchMltLeaseData(targetCd, targetYmd);
                rawData.addAll(data); 
            }
        }
