...
import home.dto.AptResponseDTO;
import home.dto.AptBodyDTO; 
...
public class AptClient {
     ...
     */
    public List<AptItemDTO> fetchAptLeaseData(String lawdCd, String dealYmd) {
        ...
        try {
            ...
            // 3. XML 파싱 (최종 파싱 로직)
            AptResponseDTO responseDTO = xmlMapper.readValue(xmlResult, AptResponseDTO.class);
            
            // 4. 데이터 추출 및 반환 (BodyDTO의 item 리스트를 바로 가져옴)
            return Optional.ofNullable(responseDTO)
                    .map(AptResponseDTO::getBody)
                    .map(AptBodyDTO::getItem)
